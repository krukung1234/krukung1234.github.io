...
const vipLoginBox = document.getElementById("vipLoginBox");
...
function shortText(text="",max=72){const t=String(text||"").replace(/\s+/g," " ).trim();return t.length>max?t.slice(0,max).trim()+"...":t}
...
function vipLogin(){
  const ok = (vipPassword?.value || "") === window.APP_CONFIG.VIP_PASSWORD;
  if(!ok){
    if(vipError) vipError.textContent = "รหัส VIP ไม่ถูกต้อง";
    vipPassword?.focus();
    return;
  }
...
function vipLogout(){localStorage.removeItem("vip_ok"); location.reload()}
function showVipSite(){
  vipLoginBox?.classList.add("hidden");
  siteShell?.classList.remove("hidden");
  load();
}
vipPassword?.addEventListener("keydown", e=>{if(e.key==="Enter") vipLogin()});
...
if(localStorage.getItem("vip_ok")==="1") showVipSite(); else vipPassword?.focus();
